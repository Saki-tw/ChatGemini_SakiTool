...
class CacheManager:
    ...
    def create_cache(
        ...
        if not self._check_model_support(model):
            console.print(f"[#E8C4F0]{safe_t('cache.warning_model_not_support', fallback='警告:{model} 可能不支援 Context Caching', model=model)}[/#E8C4F0]")

        # 檢查最低 token 要求
        min_tokens = MIN_TOKENS.get(model, 1024)
        console.print(f"\n[#E8C4F0]{safe_t('cache.create_title', fallback='📦 建立 Context Cache')}[/#E8C4F0]")
        ...
        try:
            ...
            # 建立快取
            console.print(f"\n[#E8C4F0]{safe_t('cache.creating', fallback='⏳ 建立中...')}[/#E8C4F0]")
            ...
            if 'token' in error_str and 'minimum' in error_str:
                console.print(f"\n[#E8C4F0]{safe_t('cache.hint_content_too_short', fallback='提示:內容可能少於最低 {min_tokens} tokens', min_tokens=min_tokens)}[/#E8C4F0]")
                console.print(f"[#E8C4F0]{safe_t('cache.hint_increase_content', fallback='請增加內容長度以使用 Context Caching')}[/#E8C4F0]")
            elif 'not support' in error_str:
                console.print(f"\n[#E8C4F0]{safe_t('cache.hint_model_not_support', fallback='提示:{model} 可能不支援 Context Caching', model=model)}[/#E8C4F0]")
    ...
    def query_with_cache(
        ...
        if not cache:
            # 嘗試列出並查找
            console.print(f"[#E8C4F0]{safe_t('cache.trying_api', fallback='在本地找不到快取,嘗試從 API 獲取...')}[/#E8C4F0]")
        ...
        console.print(f"\n[#E8C4F0]{safe_t('cache.query_title', fallback='🔍 使用快取查詢')}[/#E8C4F0]")
        ...
        try:
            ...
            console.print(f"[#E8C4F0]{safe_t('cache.using_cache_label', fallback='Gemini (使用快取):')}[/#E8C4F0]")
    ...
    def list_caches(self) -> List[Any]:
        """列出所有快取"""
        console.print(f"\n[#E8C4F0]{safe_t('cache.list_title', fallback='📦 已建立的 Context Caches:')}[/#E8C4F0]\n")

        try:
            caches = list(client.caches.list())

            if not caches:
                console.print(f"[#E8C4F0]{safe_t('cache.no_caches_found', fallback='沒有找到快取')}[/#E8C4F0]")
                return []
    ...
    def show_savings_report(
        ...
        panel_content = f"""
[#E8C4F0]模型:[/#E8C4F0] {model}
[#E8C4F0]快取 Tokens:[/#E8C4F0] {result['cached_tokens']:,}
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        console.print("\n[bold #E8C4F0]Gemini Context Caching 管理器[/bold #E8C4F0]\n")
