...
def test_template_engine():
    """測試 TemplateEngine"""
    console.print("\n[bold cyan]測試 1：Template Engine[/bold cyan]")

    try:
        engine = TemplateEngine()
        console.print("[green]✓ TemplateEngine 初始化成功[/green]")
        ...
        console.print("[green]✓ 條件邏輯測試通過[/green]")
        ...
        console.print("[green]✓ 迴圈處理測試通過[/green]")
...
def test_template_library():
    """測試 TemplateLibrary"""
    console.print("\n[bold cyan]測試 2：Template Library[/bold cyan]")

    try:
        library = TemplateLibrary()
        console.print("[green]✓ TemplateLibrary 初始化成功[/green]")

        # 檢查內建模板
        templates = library.list_templates()
        assert len(templates) > 0, "沒有內建模板"
        console.print(f"[green]✓ 找到 {len(templates)} 個內建模板[/green]")

        # 測試取得模板
        python_func = library.get_template('python_function')
        assert python_func is not None, "python_function 模板不存在"
        console.print("[green]✓ 內建模板可正常取得[/green]")
...
def test_command_registry():
    """測試 CommandRegistry"""
    console.print("\n[bold cyan]測試 3：Command Registry[/bold cyan]")

    try:
        # 使用臨時目錄
        with tempfile.TemporaryDirectory() as temp_dir:
            registry = CommandRegistry(config_dir=temp_dir)
            console.print("[green]✓ CommandRegistry 初始化成功[/green]")
...
def test_builtin_commands():
    """測試 BuiltinCommands"""
    console.print("\n[bold cyan]測試 4：Built-in Commands[/bold cyan]")

    try:
        # 取得所有內建命令
        commands = BuiltinCommands.get_all_commands()
        assert len(commands) > 0, "沒有內建命令"
        console.print(f"[green]✓ 找到 {len(commands)} 個內建命令[/green]")
        ...
        console.print("[green]✓ 所有內建命令結構正確[/green]")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            console.print(f"[green]✓ 成功註冊 {count} 個內建命令[/green]")
            ...
            console.print("[green]✓ 內建命令執行成功[/green]")
...
def test_import_export():
    """測試命令導入/匯出"""
    console.print("\n[bold cyan]測試 5：命令導入/匯出[/bold cyan]")
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            console.print("[green]✓ 導入內容正確[/green]")
...
def test_default_commands_config():
    """測試預設命令配置檔"""
    console.print("\n[bold cyan]測試 6：預設命令配置檔[/bold cyan]")
    ...
    try:
        ...
        if not config_file.exists():
            console.print(f"[yellow]⚠️  配置檔不存在：{config_file}[/yellow]")
            console.print("[green]✓ 跳過此測試[/green]")
            return True
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            console.print(f"[green]✓ 從配置檔導入 {count} 個命令[/green]")
            ...
            console.print("[green]✓ 配置檔中的命令可正常執行[/green]")
...
def main():
    """執行所有測試"""
    console.print("\n" + "=" * 70)
    console.print("[bold cyan]CodeGemini Custom Commands - 測試套件[/bold cyan]")
    console.print("=" * 70)
