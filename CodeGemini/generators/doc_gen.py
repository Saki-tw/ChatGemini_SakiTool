...
class DocumentationGenerator:
    ...
    def scan_project(self, exclude_dirs: Optional[List[str]] = None) -> None:
        ...
        """
        console.print(f"\n[cyan]🔍 掃描專案：{self.project_path}[/cyan]")
        ...
        console.print(f"[green]✓ 發現 {len(python_files)} 個 Python 檔案[/green]")
        ...
        console.print(f"[green]✓ 分析完成：{len(self.modules)} 個模組[/green]")
    ...
    def _analyze_module(self, file_path: Path) -> Optional[ModuleInfo]:
        ...
        try:
            ...
            console.print(f"[yellow]警告：無法分析 {file_path} - {e}[/yellow]")
            return None
    ...
    def generate_readme(self, output_path: Optional[str] = None) -> str:
        ...
        if output_path:
            ...
            console.print(f"[green]✓ README 已儲存：{output_path}[/green]")
    ...
    def generate_api_docs(self, output_path: Optional[str] = None) -> str:
        ...
        if output_path:
            with open(output_path, 'w', encoding='utf-8') as f:
                f.write(api_docs)
            console.print(f"[green]✓ API 文檔已儲存：{output_path}[/green]")
...
def main():
    ...
    console.print("\n[bold cyan]CodeGemini Documentation Generator[/bold cyan]\n")
