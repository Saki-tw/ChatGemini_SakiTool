...
class ApprovalWorkflow:
    ...
    def show_diff_preview(self, changes: List[FileChange]) -> None:
        ...
        table.add_column("動作", style="#B565D8", width=max(10, int(console_width * 0.10)))
        table.add_column("檔案路徑", style="white")
        table.add_column("變更描述", style="green")
        table.add_column("預估行數", justify="right", style="#B565D8")
    ...
    def show_risk_warning(self, risk_level: RiskLevel) -> None:
        ...
        if risk_level == RiskLevel.HIGH:
            ...
            console.print(Panel(
                "[bold #B565D8]⚠️  中等風險[/bold #B565D8]\n\n"
                "此操作會修改現有檔案：\n"
                "• 建議先檢查變更內容\n"
                "• 確保有備份或版本控制",
                border_style="#B565D8",
    ...
    def wait_for_confirmation(
        ...
        while True:
            ...
            if allow_preview and all_changes:
                options.append("[bold #B565D8]p[/bold #B565D8] - 預覽變更")

            if allow_step_by_step and len(plan.steps) > 1:
                options.append("[bold #B565D8]s[/bold #B565D8] - 分步執行")
    ...
    def _step_by_step_confirmation(self, request: ApprovalRequest) -> bool:
        ...
        for step in plan.steps:
            ...
            if approved:
                ...
                console.print(safe_t('codegemini.approval.step_approved', fallback='  [#B565D8]✓ 步驟 {num} 已批准[/#B565D8]', num=step.step_number))
            else:
                console.print(safe_t('codegemini.approval.step_skipped', fallback='  [dim #B565D8]✗ 步驟 {num} 已跳過[/red]', num=step.step_number))
    ...
    def execute_approved_plan(
        ...
        if request.status != ApprovalStatus.APPROVED:
            console.print(safe_t('codegemini.approval.not_approved', fallback='[dim #B565D8]錯誤：計畫尚未被批准[/red]'))
            return False

        console.print(safe_t('codegemini.approval.executing', fallback='\n[bold #B565D8]🚀 開始執行計畫...[/bold #B565D8]\n'))
        ...
        if executor:
            ...
            for step in request.plan.steps:
                console.print(safe_t("approval.execute.step", fallback="\n[#B565D8]執行步驟 {num}：[/#B565D8]{desc}").format(num=step.step_number, desc=step.description))

                # 模擬執行
                import time
                time.sleep(0.5)

                console.print(safe_t("approval.execute.completed", fallback="  [#B565D8]✓ 完成[/#B565D8]"))

            console.print(safe_t('codegemini.approval.simulation_complete', fallback='\n[bold green]✅ 所有步驟已執行（模擬模式）[/bold green]'))
            console.print(safe_t('codegemini.approval.requires_integration', fallback='[#B565D8]注意：實際執行需要整合 MultiFileEditor 模組[/#B565D8]'))
...
def main():
    ...
    console.print(safe_t("approval.test.title", fallback="[bold #B565D8]CodeGemini Approval Workflow 測試[/bold #B565D8]\n"))
