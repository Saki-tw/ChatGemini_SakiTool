...
#### 💰 自動省錢 75-90%
智能快取系統自動累積對話建立快取，一次查詢就回本，完全無腦省錢。

**→ 詳見：自動快取管理（v1.0.3 新增功能）**

#### 🗄️ 輕量向量資料庫
無需安裝大型資料庫，即開即用，程式碼與對話自動索引，超快速語意搜尋。

**→ 詳見：完整功能清單**

#### 🌍 多語言介面支援
四種語言即時切換（繁中、英、日、韓），翻譯品質經過 96.5% 測試驗證。

**→ 詳見：v1.0.5 版本更新**

#### 🔌 MCP 智慧整合系統
7 種強大工具自動偵測啟用，零配置使用，87.5% 測試通過率。
...
完全自動化，支援 macOS/Linux，安裝完成後任意位置輸入 `ChatGemini` 即可啟動。
...
### 📅 v1.0.5（2025-10-29）— 最新版本

**三大進展**：

- ✨ **MCP 智慧整合**：7 種工具（檔案、Git、網頁搜尋、序列推理等）、自動偵測啟用、87.5% 測試通過率

- 🌍 **多語言完善**：1,491 條繁中 + 533 條英日韓翻譯、對話中輸入 `lang` 即時切換、96.5% 測試通過率

- ⚡ **性能優化**：批次處理加速、智能快取預載、記憶體使用優化
...
- ⚙️ **智能配置管理**：三層優先權設計（系統預設 → 使用者配置 → 環境變數）

- 💾 **記憶體智慧管理**：對話自動存檔，保留最新 50 條，舊資料自動歸檔

- 🛡️ **錯誤處理強化**：智能診斷 + 預防性檢查 + 自動修復建議
...
- 🎨 **進階文字介面**：方向鍵瀏覽歷史、Tab 自動補全、多行輸入支援

- 🧠 **動態思考模式**：`[think:5000]` 固定預算、`[think:auto]` 自動決定、`[no-think]` 關閉思考

- 💰 **自動快取系統**：累積達 5000 tokens 自動建立快取，即時顯示成本分析

- 📎 **智慧檔案附加**：自動判斷 30+ 格式文字檔 vs 媒體檔，正確處理

- 💡 **互動式幫助**：內建 6 大主題選單，輸入 `help` 即可查看

- 💵 **新台幣計價**：每次對話即時顯示 NT$ 花費
...
- 💬 **智能對話**：串流輸出、200 萬 tokens 支援、完美中文處理

- 📷 **圖像理解**：圖像描述、OCR 文字提取、物體偵測、圖像比較、視覺問答、批次處理

- 📹 **影片理解**：影片分析、多格式支援、智慧處理、互動對話、最長 2 小時

- 🎬 **影片生成**：Veo 3.1 文字轉影片、8 秒 720p/1080p、原生音訊、參考圖片引導

- 🎞️ **影片編輯**：Flow Engine 自然語言編輯（場景偵測、智慧裁切、濾鏡、速度調整、浮水印）

- 🎵 **音訊處理**：提取、合併、音量調整、淡入淡出、背景音樂混音

- 📝 **字幕生成**：語音辨識、多語言翻譯、SRT/VTT 格式、字幕燒錄

- 🖼️ **圖片生成**：Imagen 文字轉圖片、編輯、放大、多種長寬比
